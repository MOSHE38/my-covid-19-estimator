const covid19ImpactEstimator = (data) => {
  ...
  const impactDollarsInFlight = (data) => {
    const iDI = Math.trunc(((data.reportedCases * 10 * 2 ** factoriser(data)) * 0.65 * data.region.avgDailyIncomeInUSD) / 30);
    return iDI
  }
  const severeImpactDollarsInFlight = (data) => {
    const sIDI = Math.trunc(((data.reportedCases * 50 * 2 ** factoriser(data)) * 0.65 * data.region.avgDailyIncomeInUSD) / 30);
    return sIDI
  }
  const covid19ImpactEstimator = (data) => ({
    data,
    impact: {
      ...
      casesForICUByRequestedTime: 0.05 * (data.reportedCases * 10 * 2 ** factoriser(data)),
      casesForVentilatorsByRequestedTime: 0.02 * (data.reportedCases * 10 * 2 ** factoriser(data)),
    ...
    severeImpact: {
      ...
      casesForICUByRequestedTime: 0.05 * (data.reportedCases * 50 * 2 ** factoriser(data)),
      casesForVentilatorsByRequestedTime: 0.02 * (data.reportedCases * 50 * 2 ** factoriser(data)),
